            ...
            <div class="grid gap-4">
                <form @submit.prevent="calculate" class="flex items-center gap-2">
                    <input
                        ...
                        type="text"
                    />
                    <button type="submit" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-sky-300 text-sky-300-foreground hover:bg-sky-300/90 h-10 px-4 py-2">
                        Calculate
                    </button>
                </form>
                ...
                <p v-show="calculatorForm.hasErrors && calculatorForm.errors.expression" class="text-red-600">{{ calculatorForm.errors.expression }}</p>
                ...
                <div v-if="history.length > 0" class="border-t border-gray-200 dark:border-gray-700 pt-4">
                    ...
                    <div class="grid gap-2">
                        <div v-for="item in history" :key="item.id" class="flex items-center justify-between bg-gray-100 dark:bg-gray-800 rounded-md p-2">
